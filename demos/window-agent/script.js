/**
 * Copyright 2026 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */

import { GoogleGenAI } from "https://esm.sh/@google/genai";

const setupContainer = document.getElementById("setup-container");
const chatContainer = document.getElementById("chat-container");
const apiKeyInput = document.getElementById("api-key-input");
const chatWindow = document.getElementById("chat-window");
const userInput = document.getElementById("user-input");
const sendBtn = document.getElementById("send-btn");
const saveKeyBtn = document.getElementById("save-key-btn");
const logoutBtn = document.getElementById("logout-btn");
const urlInput = document.getElementById("url-input");
const openPopupBtn = document.getElementById("open-popup-btn");
const openPipBtn = document.getElementById("open-pip-btn");
const iframe = document.getElementById("iframe");
const popupStatusIcon = document.getElementById("popup-status-icon");
const popupStatusTitle = document.getElementById("popup-status-title");
const popupStatusDesc = document.getElementById("popup-status-desc");
const popupActions = document.getElementById("popup-actions");
const focusPopupBtn = document.getElementById("focus-popup-btn");
const refreshToolsBtn = document.getElementById("refresh-tools-btn");

let ai, chat;
let popupWindow = null;
let pipWindow = null;

function updatePopupStatus() {
  const isPipOpen = pipWindow && !pipWindow.closed;
  const isPopupOpen = popupWindow && !popupWindow.closed;

  if (isPipOpen || isPopupOpen) {
    popupStatusIcon.textContent = "📺";
    popupStatusTitle.textContent = "Window opened";
    popupStatusDesc.textContent = `Interact with the web page from the chat.`;
    popupActions.classList.remove("hidden");
  } else {
    popupStatusIcon.textContent = "🪟";
    popupStatusTitle.textContent = "No Active Window";
    popupStatusDesc.textContent =
      'Click "Open Popup" or "Open PiP window" to launch the WebMCP demo in a separate window.';
    popupActions.classList.add("hidden");
  }
}

async function logExposedTools() {
  updatePopupStatus();
  const hasWindow =
    (pipWindow && !pipWindow.closed) || (popupWindow && !popupWindow.closed);
  if (!hasWindow) {
    return;
  }
  const tools = await getTools();
  const url = urlInput.value.trim();
  appendMessage(
    "System",
    `🌐 ${tools.length} tools are exposed by ${url}`,
    "tool-indicator",
  );
}

async function getTools() {
  const popupOrigin = new URL(urlInput.value.trim()).origin;
  const tools = await document.modelContext.getTools({
    fromOrigins: [popupOrigin],
  });
  return tools;
}

async function getConfig() {
  const tools = await getTools();

  const systemInstruction = [
    "You are an assistant embedded in a web page.",
    "CRITICAL RULE: Do not try to use other tools than the available ones.",
  ];

  const functionDeclarations = tools.map((tool) => ({
    name: tool.name,
    description: tool.description,
    parametersJsonSchema: tool.inputSchema
      ? JSON.parse(tool.inputSchema)
      : { type: "object", properties: {} },
  }));

  return { systemInstruction, tools: [{ functionDeclarations }] };
}

const storedKey = localStorage.getItem("gemini_api_key");
if (storedKey) {
  initChat(storedKey);
} else {
  setupContainer.classList.remove("hidden");
}

saveKeyBtn.addEventListener("click", () => {
  const key = apiKeyInput.value.trim();
  if (key) {
    localStorage.setItem("gemini_api_key", key);
    initChat(key);
    const hasWindow =
      (pipWindow && !pipWindow.closed) || (popupWindow && !popupWindow.closed);
    if (!hasWindow) {
      openOrNavigatePopup();
    }
  }
});

function initChat(apiKey) {
  ai = new GoogleGenAI({ apiKey: apiKey });
  setupContainer.classList.add("hidden");
  chatContainer.classList.remove("hidden");
  userInput.focus();
}

urlInput.addEventListener("keypress", (e) => {
  if (e.key !== "Enter") return;
  openOrNavigatePopup();
});

openPopupBtn.addEventListener("click", openOrNavigatePopup);
openPipBtn.addEventListener("click", openOrNavigatePip);

focusPopupBtn.addEventListener("click", () => {
  if (pipWindow && !pipWindow.closed) {
    pipWindow.focus();
  } else if (popupWindow && !popupWindow.closed) {
    popupWindow.focus();
  }
});

refreshToolsBtn.addEventListener("click", logExposedTools);
iframe.addEventListener("load", logExposedTools);

function closePipWindow() {
  if (pipWindow && !pipWindow.closed) {
    try {
      const returnedIframe = window.document.adoptNode(iframe);
      returnedIframe.classList.add("hidden");
      document.getElementById("browser-container").append(returnedIframe);
    } catch (e) {}
    pipWindow.close();
    pipWindow = null;
  }
}

async function openOrNavigatePopup() {
  closePipWindow();
  const url = urlInput.value.trim();
  urlInput.value = url;
  chatWindow.innerHTML = "";
  chat = null;
  if (!popupWindow || popupWindow.closed) {
    popupWindow = window.open(url, "popup", "width=600,height=600");
    const checkClosed = setInterval(() => {
      if (popupWindow.closed) {
        clearInterval(checkClosed);
        popupWindow = null;
        updatePopupStatus();
      }
    }, 500);
  } else {
    popupWindow.location.href = url;
    popupWindow.focus();
  }
  updatePopupStatus();
  urlInput.blur();

  await logExposedTools();
}

async function openOrNavigatePip() {
  if (!("documentPictureInPicture" in window)) {
    appendMessage(
      "System",
      "⚠️ Document Picture-in-Picture Web API is not supported in this browser.",
      "error",
    );
    return;
  }

  const url = urlInput.value.trim();
  urlInput.value = url;
  chatWindow.innerHTML = "";
  chat = null;

  if (popupWindow && !popupWindow.closed) {
    popupWindow.close();
    popupWindow = null;
  }

  if (!pipWindow || pipWindow.closed) {
    pipWindow = await window.documentPictureInPicture.requestWindow({
      width: 1000,
      height: 800,
    });

    pipWindow.document.body.style.margin = "0";
    pipWindow.document.body.style.overflow = "hidden";

    const adoptedIframe = pipWindow.document.adoptNode(iframe);
    adoptedIframe.classList.remove("hidden");
    adoptedIframe.style.width = "100vw";
    adoptedIframe.style.height = "100vh";
    adoptedIframe.style.border = "none";
    adoptedIframe.allow = `tools ${new URL(url).origin}`;
    adoptedIframe.src = url;
    pipWindow.document.body.append(adoptedIframe);

    pipWindow.addEventListener("pagehide", () => {
      try {
        const returnedIframe = window.document.adoptNode(iframe);
        returnedIframe.src = "about:blank";
        returnedIframe.classList.add("hidden");
        document.getElementById("browser-container").append(returnedIframe);
      } catch (e) {}
      pipWindow = null;
      updatePopupStatus();
    });
  } else {
    iframe.src = url;
    pipWindow.focus();
  }

  updatePopupStatus();
  urlInput.blur();
}

updatePopupStatus();

logoutBtn.addEventListener("click", () => {
  localStorage.removeItem("gemini_api_key");
  ai = null;
  chatWindow.innerHTML = "";
  document.body.style.backgroundColor = "";
  setupContainer.classList.remove("hidden");
  chatContainer.classList.add("hidden");
});

sendBtn.addEventListener("click", handleUserSubmit);
userInput.addEventListener("keypress", (e) => {
  if (e.key === "Enter" && !userInput.disabled) handleUserSubmit();
});

async function handleUserSubmit() {
  try {
    const text = userInput.value.trim();
    if (!text || !ai) return;

    const hasWindow =
      (pipWindow && !pipWindow.closed) || (popupWindow && !popupWindow.closed);
    if (!hasWindow) {
      appendMessage(
        "System",
        '⚠️ No active window! Please click "Open Popup" or "Open PiP window" to launch the target website.',
        "error",
      );
      return;
    }

    userInput.value = "";
    userInput.disabled = true;
    sendBtn.disabled = true;

    appendMessage("You", text, "user");

    chat ??= ai.chats.create({ model: "gemini-3.5-flash" });

    const sendMessageParams = { message: text, config: await getConfig() };
    let currentResult = await chat.sendMessage(sendMessageParams);
    let finalResponseGiven = false;

    while (!finalResponseGiven) {
      const response = currentResult;
      const functionCalls = response.functionCalls || [];

      if (functionCalls.length === 0) {
        appendMessage("Agent", response.text, "agent");
        finalResponseGiven = true;
      } else {
        const toolResponses = [];
        for (const { name, args } of functionCalls) {
          const inputArgs = JSON.stringify(args);
          try {
            appendMessage(
              "System",
              `⚙️ Executing tool: ${name}...`,
              "tool-indicator",
            );
            const tools = await getTools();
            const tool = tools.find((t) => t.name == name);
            const result = await document.modelContext.executeTool(
              tool,
              inputArgs,
            );

            toolResponses.push({
              functionResponse: { name, response: { result } },
            });
          } catch (error) {
            appendMessage("System", `Error: ${error.message}`, "error");
            toolResponses.push({
              functionResponse: { name, response: { error: error.message } },
            });
          }
        }
        const sendMessageParams = {
          message: toolResponses,
          config: await getConfig(),
        };
        currentResult = await chat.sendMessage(sendMessageParams);
      }
    }

    userInput.disabled = false;
    sendBtn.disabled = false;
    userInput.focus();
  } catch (error) {
    appendMessage("System", `Error: ${error.message}`, "error");
  }
}

function appendMessage(sender, text, className) {
  const msgDiv = document.createElement("div");
  msgDiv.className = `message ${className}`;
  msgDiv.innerHTML = text.replace(/\n/g, "<br>");
  chatWindow.appendChild(msgDiv);
  chatWindow.scrollTop = chatWindow.scrollHeight;
}
