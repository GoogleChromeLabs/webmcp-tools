...
- [SitemapKit](https://sitemapkit.com/webmcp-sitemap-tools) - A live sitemap discovery and validation site that registers `find_sitemaps` and `check_sitemap` WebMCP tools. Anonymous calls return real results such as sitemap locations, URL counts, metadata, and validation errors.
  - **Example Prompt:** "Find the XML sitemaps for example.com, then check its sitemap.xml for validation errors."
